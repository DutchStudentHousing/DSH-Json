...
def generate_fake_messages(cursor):
    try:
        ...
        for record in landlord_records:
            ...
            cursor.execute("SELECT EXTRACT(EPOCH FROM TIMESTAMP '2019-01-01' - %s);", (listing_date,))
            ...
            for tenant_id in random.sample(tenant_ids, 5):  # Choose 5 tenants to send messages
                ...
                message = {
                    ...
                    "sent_at": timestamp.strftime('%Y-%m-%dT%H:%M:%S')
                }
                ...
                if random.choice([True, False]):  # Randomly decide whether the landlord replies
                    ...
                    reply = {
                        ...
                        "sent_at":  timestamp_reply.strftime('%Y-%m-%dT%H:%M:%S')  # date in dd-mm-yyyy time format
                    }
                    ...
                    if random.choice([True, False]):  # Randomly decide whether there's a third message
                        ...
                        second_message = {
                            ...
                            "sent_at": timestamp_second_message.strftime('%Y-%m-%dT%H:%M:%S')  # date in dd-mm-yyyy time format
                        }
