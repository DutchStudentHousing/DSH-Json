...
db = psycopg2.connect(
    host='localhost',  # host = db # local os.environ.get('DSH_HOST')
    port=5432,  # post = 5432 # local os.environ.get('DSH_PORT')
    database='DSH',  # dsh = dsh # local os.environ.get('DSH_DB')
    user='postgres',  # user = dsh # local os.environ.get('DSH_USER')
    password='Diesel2!',  # pw = dsh # local os.environ.get("DSH_PASSWORD")
)
